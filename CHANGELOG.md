...
- The binary ZIP, binary checksum, source ZIP, and source checksum are promoted as one recoverable set from stable non-write-sharing input leases. Strict sidecar filename/hash pairing is bound to the final gated ZIP hashes, output bytes are durably flushed and leased/reverified, complete prior sets roll back byte-for-byte on failure, ordinary pre-promotion failures leave no transaction residue, and partial prior sets or true stale transactions fail closed.
