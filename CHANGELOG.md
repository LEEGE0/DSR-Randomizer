...
- The builder now creates and validates both inner ZIPs privately, binds their exact bytes in the outer hash manifest, and publishes only the one leased outer file under a canonical output-root lock. A same-filesystem pending file is durably written and fully revalidated through its exact still-open Windows handle; handle-based atomic rename is the sole commit point. Pre-commit faults leave old bytes or first-publish absence unchanged. There is no backup or rollback namespace after commit: the renamed final handle remains live through repeated identity/hash/outer checks and return, nonessential post-commit faults warn while preserving the committed new archive, and an adversarial hard-link alias fails closed by removing only the canonical link through that handle. Concurrent publishers receive `PUBLICATION_IN_PROGRESS`; legacy cleanup derives only the four exact former loose ZIP/sidecar paths from a validated version and preserves unsafe links.
