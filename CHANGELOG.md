...
- The builder now creates and validates both inner ZIPs privately, binds their exact bytes in the outer hash manifest, and publishes only the one leased outer file under a canonical output-root lock. A same-filesystem pending file is durably written, verified, and renamed through its exact still-open Windows handle; the final handle remains live through verification and whole-backup cleanup. Pre-replace faults leave the old file, post-replace verification faults restore the exact leased prior bytes through a verified handle-owned candidate, concurrent publishers receive `PUBLICATION_IN_PROGRESS`, and no transaction directory or journal is used. Legacy cleanup derives only the four exact former loose ZIP/sidecar paths from a validated version and preserves unsafe links.
