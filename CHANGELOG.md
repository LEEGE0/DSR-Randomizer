...
- The builder now creates and validates both inner ZIPs privately, binds their exact bytes in the outer hash manifest, and publishes only the one leased outer file under a canonical output-root lock. A same-filesystem pending file is durably written and fully revalidated through its exact still-open Windows handle; successful handle-based atomic rename is recorded immediately as the sole commit point. Pre-commit faults leave old bytes or first-publish absence unchanged. There is no backup or rollback namespace after commit. Before the final observation, all staged, lock, root, and non-final wrappers are explicitly closed and cleared. With only the canonical handle left, SHA-256/outer validation and exact-path resolution precede a final point-in-time regular/non-reparse/single-link observation. Delayed hard links during the scan or at former unwind seams are rejected fail-closed. The observation and later managed handle close are not atomic: arbitrary same-user mutation after the observation, including that instruction interval and post-publication changes, is outside the threat model. The output-root lock serializes official publishers only. Distributors must keep other processes from modifying `artifacts` during publication and recompute the outer SHA-256 immediately before transferring the verified bytes. Legacy cleanup derives only the four exact former loose ZIP/sidecar paths from a validated version and preserves unsafe links.
