...
- 마지막 package/privacy/source gate 뒤 두 내부 ZIP의 expected SHA-256을 고정하고, 두 exact leased byte와 strict LF-only `SHA256SUMS.txt`로 deterministic outer ZIP을 만든다. 게시 완료까지 canonical output root의 빈 `.dsr-release-publication.lock`을 배타적으로 유지하고, 경쟁 publisher는 canonical 출력에 손대지 않은 채 `PUBLICATION_IN_PROGRESS`로 실패한다. 게시자는 gated outer를 안정된 lease로 열고 같은 volume의 pending file을 READ/DELETE 권한과 write/delete 공유 거부를 가진 handle로 생성한다. 그 한 handle로 byte 복사, `Flush(true)`, SHA-256/outer semantics 검증을 끝낸 뒤 `SetFileInformationByHandle(FileRenameInfo)`로 바로 그 객체를 canonical 이름에 배치한다. Rename된 final handle은 backup 정리와 성공 반환 직전까지 계속 열려 있으므로 교체가 차단된다. 기존 outer가 있으면 전체 한 파일을 lease/검증해 `.previous` whole-file backup으로 복사하고 그 exact backup handle도 rollback 가능 구간 동안 유지한다. Replace 전 실패는 기존 outer를 유지한다. Replace 후 검증 실패는 backup의 leased byte를 새로운 handle-owned rollback candidate에 스트리밍/flush/사전 검증하고, 실패한 canonical도 exact handle로 격리한 뒤 rollback candidate 자체를 handle-rename해 복원한다. Backup 정리 실패는 valid canonical outer를 손상하지 않으며 valid `.previous`만 남길 수 있다. Transaction directory와 journal은 없다. 성공 뒤 legacy cleanup은 caller path/name을 받지 않고 검증된 version에서 과거 네 loose ZIP/sidecar exact path만 도출하며 regular/single-link/non-reparse일 때만 삭제한다.
