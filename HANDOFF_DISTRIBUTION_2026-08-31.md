...
- 마지막 package/privacy/source gate 뒤 두 ZIP의 expected SHA-256을 고정하고, 네 staged 입력을 각각 한 번 열어 write/delete 교체를 막는 lease를 게시 완료까지 유지한다. Sidecar는 정확한 소문자 SHA-256, 두 칸, 대응 ZIP 파일명, LF만 허용한다. 기존 네 파일이 모두 있으면 전부 backup한 뒤 leased byte만 최종 경로에 쓰고 내구성 flush하며, 최종 네 파일을 다시 lease해 expected hash와 sidecar 관계를 검증한 상태로 backup/journal을 정리한다. 게시 전 open/hash/journal 실패도 고유 transaction을 제거하며, 게시 뒤 실패는 새 파일만 제거하고 기존 네 파일을 복구한다. 기존 파일이 일부만 있거나 이전 프로세스가 남긴 진짜 stale transaction이 있으면 fail-closed다.
