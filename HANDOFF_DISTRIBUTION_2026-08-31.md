...
- 마지막 package/privacy/source gate 뒤 두 내부 ZIP의 expected SHA-256을 고정하고, 두 exact leased byte와 strict LF-only `SHA256SUMS.txt`로 deterministic outer ZIP을 만든다. 게시 완료까지 canonical output root의 빈 `.dsr-release-publication.lock`을 배타적으로 유지하고, 경쟁 publisher는 canonical 출력에 손대지 않은 채 `PUBLICATION_IN_PROGRESS`로 실패한다. 게시자는 gated outer를 안정된 lease로 열고 같은 volume의 pending file을 READ/DELETE 권한과 write/delete 공유 거부를 가진 handle로 생성한다. 그 한 handle로 byte 복사와 `Flush(true)`를 끝낸다. `BeforeHandleRename` hook 뒤 즉시 같은 handle의 regular/non-reparse/single-link identity, expected SHA-256, exact outer semantics를 다시 확인한다. 그 뒤 `SetFileInformationByHandle(FileRenameInfo)`로 바로 그 검증된 객체를 canonical 이름에 배치하는 순간이 유일한 commit point다. 그 전 실패는 기존 canonical을 byte-exact하게 보존하거나 첫 게시의 부재를 유지한다. Commit 뒤에는 whole backup, rollback candidate, failed-canonical 이름, transaction directory, journal이 모두 없다. Rename된 final handle을 post-commit identity/hash/outer 재검증 두 번과 성공 반환 직전까지 계속 열어 write/delete 교체를 막는다. 비필수 post-commit hook 실패는 warning으로 보고하고 exact new canonical을 유지한다. Mandatory post-rename 검사 전에 hostile hard-link alias가 추가되면 arbitrary pathname rollback을 하지 않고 같은 handle로 canonical link만 제거해 fail closed한다. Handle과 publication lock을 놓은 뒤에는 동일 사용자 권한의 다른 프로세스가 파일을 바꿀 수 있으므로 보증 범위는 live success boundary까지다. 성공 뒤 legacy cleanup은 caller path/name을 받지 않고 검증된 version에서 과거 네 loose ZIP/sidecar exact path만 도출하며 regular/single-link/non-reparse일 때만 삭제한다.
