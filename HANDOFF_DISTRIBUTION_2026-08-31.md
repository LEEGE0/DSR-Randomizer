...
- 마지막 package/privacy/source gate 뒤 두 내부 ZIP의 expected SHA-256을 고정하고, 두 exact leased byte와 strict LF-only `SHA256SUMS.txt`로 deterministic outer ZIP을 만든다. Canonical output root의 빈 `.dsr-release-publication.lock`은 공식 publisher 인스턴스끼리만 직렬화한다. 잠금을 만난 경쟁 cooperative publisher는 canonical을 바꾸지 못하고 `PUBLICATION_IN_PROGRESS`로 실패하지만, 이 잠금은 임의의 동일 OS 사용자 프로세스에 대한 보안 경계가 아니다. 게시자는 gated outer를 안정된 lease로 열고 같은 volume의 pending file을 READ/DELETE 권한과 write/delete 공유 거부를 가진 handle로 생성한다. 그 한 handle로 byte 복사와 `Flush(true)`를 끝낸다. `BeforeHandleRename` hook 뒤 즉시 같은 handle의 regular/non-reparse/single-link identity, expected SHA-256, exact outer semantics를 다시 확인한다. 그 뒤 `SetFileInformationByHandle(FileRenameInfo)`가 성공하면 fallible path 조회 전에 그 사실을 기록하며, 그 native 성공이 유일한 commit point다. 그 전 실패는 기존 canonical을 byte-exact하게 보존하거나 첫 게시의 부재를 유지한다. Commit 뒤에는 whole backup, rollback candidate, failed-canonical 이름, transaction directory, journal이 모두 없다. 최종 성공 관찰 전에 staged source lease, publication lock, output-root lease, 모든 non-final wrapper를 명시적으로 dispose/null 처리한다. 이 단계의 실패는 committed-new 실패이며 성공으로 보고하지 않는다. Canonical handle 하나만 남긴 뒤 SHA-256/outer 검사를 수행하고 exact final path를 resolve한 다음 regular/non-reparse/`NumberOfLinks == 1`을 시점 기반으로 마지막 관찰한다. 과거 각 unwind seam 또는 긴 scan 중에 생긴 hard-link는 이 관찰 전에 보이므로 canonical link를 fail closed로 제거한다. 그러나 이 file-information 관찰과 이후 managed handle close는 atomic하지 않다. 관찰 직후의 instruction interval이나 게시 뒤에 동일 OS 사용자 권한의 비협조 프로세스가 hard-link 또는 namespace/content 변경을 만드는 것은 threat model 밖이며 publisher가 막는다고 주장하지 않는다. 빌드/게시 중에는 `artifacts`를 변경하는 다른 프로세스를 실행하지 않고, 전달 직전에 outer SHA-256을 다시 계산해야 한다. 배포물은 영구 보호되는 로컬 pathname이 아니라 검증 후 전달한 exact ZIP byte와 hash다. 성공 뒤 legacy cleanup은 caller path/name을 받지 않고 검증된 version에서 과거 네 loose ZIP/sidecar exact path만 도출하며 regular/single-link/non-reparse일 때만 삭제한다.
