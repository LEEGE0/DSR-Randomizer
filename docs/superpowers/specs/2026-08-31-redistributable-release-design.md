...
The same invariant is checked after binary staging and again inside the source builder immediately before it archives committed objects. After the final package, privacy, and source checks, the builder captures both exact ZIP SHA-256 values. Publication creates one unique destination transaction, then opens all four staged inputs once with stable regular-file identities and sharing that denies write/delete replacement. Each strict lowercase sidecar must name and hash its paired ZIP, and each leased ZIP must match the gated hash. A complete prior set is backed up before only the already leased bytes are streamed to new final files and durably flushed. All four final files are then leased and their blob hashes, gated ZIP hashes, and sidecar pairings are reverified while backup/journal cleanup completes. Any ordinary failure is owned by the same rollback state machine: before outputs move it removes the exact transaction, and afterward it removes only new outputs and restores all four prior files byte-for-byte. Partial prior sets and true stale transactions from an interrupted process fail closed. Tests cover clean exact state, dirty tracked main state, a nonignored untracked compile input, an uninitialized submodule, a wrong submodule HEAD, a dirty submodule, locked staged input plus immediate retry, journal setup failure, mismatched sidecar name/hash, gated-hash mismatch, attempted staged replacement at the copy boundary, post-write tamper rollback, controlled third-copy rollback, successful first/replacement publication, partial-set rejection, and stale-transaction rejection.
