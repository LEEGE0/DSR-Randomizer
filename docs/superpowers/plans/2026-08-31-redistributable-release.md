...
Document the bundled project-owned bridge/host, the dated TPF/DrSwizzler exclusion, the absence of the unused BouncyCastle runtime, complete ZstdNet/libzstd notices, and corresponding-source obligations for the exact SoulsFormatsNEXT, ZstdNet, and Zstandard inputs. Record `0.1.0-alpha.2`, bridge auto-install/verification, third-party user-supply boundary, clean-root verification, and native test fixture repair. Do not state that the binary ZIP includes or installs recipient-supplied third-party tools.

The release helper must fail closed for dirty tracked or nonignored untracked main-repository state, an uninitialized or wrong-HEAD submodule, a dirty submodule, and any pinned-contract mismatch. It must accept ignored generated outputs. Exercise all of those states in isolated temporary Git fixtures rather than mutating the release worktree.
...
Publication threat model: the output-root lock serializes cooperative official publisher instances only. It is not an OS security boundary against an arbitrary process running as the same user. Do not run another process that modifies `artifacts` during the build or publication. Stable-handle checks detect mutations observable through the final same-handle path/regular/reparse/link-count query, but that query and managed handle close are not atomic; adversarial same-user mutation beginning after the final observation, including its instruction-to-close interval and post-publication changes, is out of scope. Recompute the reported outer SHA-256 immediately before transfer. The verified outer bytes and hash, not a perpetually protected local pathname, are the release deliverable.

- [ ] **Step 4: Build the final release from clean committed source**
...
List the outer archive and require exactly three root entries in ordinal order with fixed timestamps: source ZIP, binary ZIP, and `SHA256SUMS.txt`. Reject duplicates, traversal, aliases, or any extra entry; strictly recompute both inner hashes and compare the exact two LF-only manifest lines. On the exact embedded binary bytes, compare the 12 paths byte-for-byte with the allowlist, extract to a new temporary directory, run `--validate-package`, parse the official host's .NET v6 bundle manifest and embedded deps JSON, verify bundled dependencies have complete notices, and scan extracted bytes for reviewed privacy sentinels. On the exact embedded source bytes, require a single prefix, sorted unique safe paths, fixed timestamps, a strict main/submodule revision manifest, committed subset/main solution/build scripts, full exact pinned SoulsFormatsNEXT, ZstdNet, and Zstandard source/build/license trees, and absence of Git/build/artifact/private state. Compare every archived file with the main tracked tree plus each pinned submodule tree, extract to a short safe path, and restore/build the bridge host. Recompute and report the outer SHA-256 separately; no external sidecar exists. Exercise first publication and replacement; actual/injected handle-rename failure with exact old or absent pre-commit state; an injected final-path resolution failure after native rename that retains exact committed new bytes for first/replacement publication; pending-path replacement; hard-link insertion before the final pre-rename handle check for both existing and first publication; post-commit replacement plus hook failure while the exact new canonical remains live; synchronous and delayed asynchronous post-rename hard-link insertion; delayed hard links after staged-lease, publication-lock, and output-root-lease disposal; an explicit lifecycle trace showing every non-final resource disposed before final validation and the canonical handle released before return; expensive SHA-256/outer validation and exact-path resolution preceding the final same-handle file-information/link-count observation; synchronized official publishers with `PUBLICATION_IN_PROGRESS`/retry; exact four-function module exports; and version-derived cleanup of only the four exact obsolete loose artifact paths, including rejection/preservation of arbitrary names, the authoritative outer, and unsafe links. Preserve this evidence as cooperative/stable-handle coverage through the final observation; do not claim that the separate observation and managed close atomically exclude a later uncooperative same-user hard link. Require no backup, rollback, failed-canonical, transaction, or journal namespace.
